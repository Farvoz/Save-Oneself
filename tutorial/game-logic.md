...
**Двусторонние карты**:
   ...
   - Свойства тыльной стороны:
     ...
     - Обработчики событий: `onPlace`, `onRoundStart`, `canActivate`
   ...
   - Свойства лицевой стороны:
     ...
     - `description` — описание карты и её эффектов
     - Обработчики событий: `onPlace`, `onBeforeShipMove`, `onShipMove`, `onRoundStart`
...
\n+- При входе в состояние `playing` колода один раз перемешивается.
...
- **Входные действия:**
  ...
  - Вызов обработчика `afterPlace` (глобально), если определён
...
- **Входные действия:**
  - Уменьшение жизней на 1 через `decreaseLive()`
...
- **Автоматические переходы (после 500ms):**
  - `gameOver` (победа): если установлены `gameOverMessage` и `isVictory`
  - `gameOver` (поражение): если `checkDefeat()` возвращает true (корабль уплыл слишком далеко)
  - `startOfRound`: в остальных случаях
- **Логика:** Автоматическое движение корабля по орбите вокруг острова и проверка условий победы/поражения, которые выставляются эффектами карт в `onShipMove`

#### 10. **gameOver** - Завершение игры
...
checkingFlippable → shipMoving → startOfRound
```

### Условия завершения игры
1. **Победа:** устанавливается эффектами карт во время `onShipMove` (формируются `gameOverMessage` и `isVictory = true`)
...
### Победа
Игрок побеждает, если выполняется одно из условий (проверяется эффектами карт в `onShipMove`):
...
- События (`MOVE_PLAYER`, `ACTIVATE_CARD`, `SKIP_MOVES`, `SKIP_PHASE`) обрабатываются через `on`
...
- **`onPlace`**: Вызывается при размещении/перевороте карты (в зависимости от конкретной карты)
- **`afterPlace`**: Дополнительные эффекты сразу после обработки `onPlace`
...
- **`canActivate`**: Кастомная логика проверки возможности переворота (например, `bottle`, `telescope`, `rocks`, `map-r`, `map-c`)
